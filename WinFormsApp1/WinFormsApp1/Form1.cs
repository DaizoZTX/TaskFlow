namespace WinFormsApp1
{
    public partial class pantallaInicial : Form
    {
        ...
        //cuarto formulario
        modificacionDatosForm ModDat = new modificacionDatosForm();
        ...
        private void boton_refresh_Click(object sender, EventArgs e)
        {
            ...
            if (loginSys.tb_usuario.Text=="Admin")
            {
                boton_AdminPanel.Visible = true;
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            if (tb_tipoDePerfil.Text == "Admin")
            {
                ModDat.Show();
            }
            if (tb_tipoDePerfil.Text == "Empleados")
            {
                MessageBox.Show("EL USUARIO NO ES UN ADMINISTRADOR", "ERROR", MessageBoxButtons.OK);
            }
            if (tb_tipoDePerfil.Text == "Sesion no iniciada...")
            {
                MessageBox.Show("NO EXISTE INICIO DE SESION", "ERROR", MessageBoxButtons.OK);
            }
        }
        //Boton Panel de administrador
        private void boton_AdminPanel_Click(object sender, EventArgs e)
        {
